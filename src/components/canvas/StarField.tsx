  ...
  return new THREE.ShaderMaterial({
    ...
    vertexShader: `
      ...
      void main() {
        ...
        gl_PointSize = aSize * (42.0 / max(1.0, -mvPosition.z));
      }
    `,
    fragmentShader: `
...
export default function StarField() {
  ...
  const [heroPositions, heroColors, heroSizes] = useMemo(() => {
    ...
    authoredNodes.forEach((node, index) => {
      ...
      colors[i3 + 1] = color.g;
      colors[i3 + 2] = color.b;
      sizes[index] = index % 5 === 0 ? 0.36 : 0.22;
    });
  ...
  useFrame((state) => {
    ...
    if (constellationRef.current) {
      ...
      material.opacity = 0.22 + Math.sin(t * 0.65) * 0.055;
    }
  });
  ...
  return (
    <group>
      ...
      <lineSegments ref={constellationRef} geometry={constellationGeometry}>
        <lineBasicMaterial vertexColors transparent opacity={0.24} blending={THREE.AdditiveBlending} depthWrite={false} />
      </lineSegments>
